...
public class GameController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var gameTask = player.GetGameSummaries();
        var availableTask = player.GetAvailableGames();
        await Task.WhenAll(gameTask, availableTask);

        return Json(new object[] {gameTask.Result, availableTask.Result});
    }
    ...
    {
        var player = _grainFactory.GetGrain<IPlayerGrain>(id);
