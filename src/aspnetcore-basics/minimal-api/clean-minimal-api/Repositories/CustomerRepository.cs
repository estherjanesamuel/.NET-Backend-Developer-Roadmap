...
public class CustomerRepository : ICustomerRepository
{
    ...
    {
        using var connection = await _connectionFactory.CreateConnectionAsync();
        return await connection.QuerySingleOrDefaultAsync<CustomerDto>("SELECT * FROM Customers WHERE Id = @Id LIMIT 1", new { Id = id.ToString()});
    }
    ...
    public async Task<bool> UpdateAsync(CustomerDto customer)
    {
        ...
        var result = await connection.ExecuteAsync(
            """
            UPDATE Customers SET Username = @Username, FullName = @FullName, Email = @Email, DateOfBirth = @DateOfBirth WHERE Id = @Id
            """
        , customer);
        return result > 0;
    }
